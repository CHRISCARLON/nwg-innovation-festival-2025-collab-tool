...
class StreetAnalysis(BaseModel):
    ...
    location: List[str] = Field(
        description="Name of street and the location of the street"
    )
    key_characteristics: List[str] = Field(
        description="Key characteristics of the road network including who manages the road network"
    )
    special_designations: List[str] = Field(
        description="Special designations or restrictions present for this USRN"
    )
    past_work_history: List[str] = Field(
        description="Work history of the specific USRN. This will include last month's work info, last 12 monthsworth of work info (please always include this historical data in your response). Please tell us if any collaborative street working has been done here last month and over the last 12 months and make this clear in your response."
    )
    potential_challenges: List[str] = Field(
        description="Potential challenges or hazards present for this USRN"
    )
    summary: str = Field(
        description="Overall summary of the analysis and information your have found."
    )
...
class LandUseAnalysis(BaseModel):
    ...
    location: List[str] = Field(
        description="Name and location details of the area, including any major landmarks nearby"
    )
    property_numbers: List[str] = Field(
        description="A high-level idea of the number of properties in the area"
    )
    institutional_properties: List[str] = Field(
        description="Names of all educational, religious, and public institutions including universities, schools, churches, cathedrals"
    )
    residential_properties: List[str] = Field(
        description="Names of all types of residential buildings including private homes, student accommodation, communal living"
    )
    commercial_properties: List[str] = Field(
        description="Names of all commercial and business properties in the area"
    )
...
class CollaborativeStreetWorksAnalysis(BaseModel):
    ...
    past_work_history: List[str] = Field(
        description="Work history summary showing total completed works per promoter organization with their sector classification (Water, Gas, Electricity, Telecommunications, Highway Authority, Other). Include which organizations have done the most work on this street and their respective sectors."
    )
    potential_challenges: List[str] = Field(
        description="Potential challenges or hazards present for this USRN"
    )
    ...
    summary: str = Field(
        description="Overall summary of the analysis and please make a recommendation for collaborative street works - based on all the context you now have would you advise this on a scale of 1 (being no) to 10 (being yes absolutely)."
    )
...
async def process_with_langchain(
    ...
    model = ChatOpenAI(model="gpt-4o-mini", temperature=0.3, api_key=secret_api_key)
    ...
    match route_type:
        ...
        case RouteType.LAND_USE.value:
            ...
            Analyse the following land use data:
            {context}
            Always focus on a summary of all the information you have found.
            """
        ...
        case RouteType.COLLABORATIVE_STREET_WORKS.value:
            ...
            structured_output = model.with_structured_output(
                CollaborativeStreetWorksAnalysis
            )
            logger.info(
                "Collaborative street works analysis structured output: {structured_output}"
            )
    ...
    try:
        ...
        return {"llm_summary": response.model_dump(), "raw_data": data}
    except Exception as e:
        return {"error": f"Langchain processing failed: {str(e)}", "raw_data": data}
