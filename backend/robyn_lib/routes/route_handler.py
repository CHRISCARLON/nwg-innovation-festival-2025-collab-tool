...
class FeatureRouteHandler:
    ...
    async def get_street_info_route(self, request: Request) -> Response:
        ...
        """
        try:
            usrn = request.query_params.get("usrn", None)
            if not usrn:
            ...
            # Get bbox from USRN
            minx, miny, maxx, maxy = await self.geometry_service.get_bbox_from_usrn(
                usrn
            )
    ...
    async def get_street_info_route_llm(self, request: Request) -> Response:
        ...
        try:
            ...
            llm_summary = await self.llm_summary_service.summarize_results(
    ...
    async def get_land_use_route(self, request: Request) -> Response:
        ...
        try:
            ...
            simplified_features = (
                await self.llm_summary_service.pre_process_land_use_info(features)
            )
    ...
    async def get_collaborative_street_works_route(self, request: Request) -> Response:
        ...
        try:
            ...
            simplified_features_land = (
                await self.llm_summary_service.pre_process_land_use_info(features_land)
            )
            ...
            simplified_features_street = (
                await self.llm_summary_service.pre_process_street_info(features_street)
            )
            ...
            # LLM
            llm_summary = await self.llm_summary_service.summarize_results(
                combined_features, path_type_collab
            )
